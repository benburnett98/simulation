...
public abstract class Page {
    ...
    public Scene getScene(String name) throws IOException {
        ...
        if (name.equals("MAIN MENU")) {
            MainMenu myMenu = new MainMenu(this.myStage);
            myScene = myMenu.buildScene(GAME_SIZE, GAME_SIZE);
        }
        if (name.equals("PLAY")) {
            PlayDirectory pp = new PlayDirectory(myStage);
            myScene = pp.buildScene(Page.GAME_SIZE, Page.GAME_SIZE);
        }
        if (name.equals("GAME OF LIFE")) {
            SimView ss = new SimView(myStage, new Controller("GameOfLife", "GOLLayout1"));
            myScene = ss.buildScene(GAME_SIZE, GAME_SIZE);
        }
        if (name.equals("Custom Menu")) {
            CustomMenu cm = new CustomMenu(myStage);
            myScene = cm.buildScene(GAME_SIZE,GAME_SIZE);
        }
